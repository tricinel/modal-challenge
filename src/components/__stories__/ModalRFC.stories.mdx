...
Looking at our existing products, we've noticed some tasks the users have problems with.
...
The `<Modal />` handles its own internal state through a `ModalContext`. This will allow it to handle its own open/closed state, pass certain props to sub components that needed and handle focus.
...
This event can be disabled through the `closeOnEscape` prop.
